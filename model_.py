...
class residual_block(nn.Module):
    ...
    def __init__(self, channel, normalize=False, dropout=False, p=0.2):
...
class CNN_net(nn.Module):
    ...
    def __init__(self, normalize=False, residual_connection=True, p=0.2, channels=[3, 64, 128], dropout=True):
        ...
        for i in range(len(channels)-1):
            ...
            self.fctions.append(nor_cov(in_channel, out_channel, dropout=dropout, normalize=normalize, p=p))

            if residual_connection:
                self.fctions.append(residual_block(channel=out_channel, dropout=dropout, normalize=normalize, p=p))
            else:
                self.fctions.append(dou_cov(channel=out_channel, dropout=dropout, normalize=normalize, p=p))

            self.fctions.append(nn.MaxPool2d(2,2))

        self.fctions.append(nn.Linear(channels[-1], 200))
